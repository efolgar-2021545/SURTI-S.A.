...
export default function BalanzaDetailPage() {
    const { id } = useParams();
    const producto = balanzasData.find((item) => String(item.id) === String(id));

    if (!producto) {
        return (
            <div className="max-w-7xl mx-auto px-4 py-20 text-center">
                <h2 className="text-2xl font-bold text-[#162B4E]">Producto no encontrado</h2>
                <Link to="/productos/balanzas" className="text-[#8D0002] underline mt-4 inline-block">
                    Volver al catálogo de balanzas
                </Link>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <nav className="text-xs text-gray-500 mb-8 uppercase tracking-wider">
                ...
                <Link to="/productos/balanzas" className="hover:text-[#162B4E] ml-1">Balanzas</Link> /
